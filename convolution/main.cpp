...
void init_input(double *input_array_1, double *input_array_2, int N_max, int batch) {
    
    int N_total = pow(N_max, 3) * batch;
    #pragma omp parallel for
    for (int i=0; i< N_total; i++) {
        input_array_1[i] = (double) rand()/RAND_MAX;
        input_array_2[i] = (double) rand()/RAND_MAX;
    }
}
...
int main(int argc, char **argv) {
    ...
    double *input_array_1 = new double[N_total];
    double *input_array_2 = new double[N_total];
    double *output_fftw = new double[N_total];
    double *output_cufft = new double[N_total];
